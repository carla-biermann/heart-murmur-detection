    ...
    for row in range(0, rows, patch_height):
        for col in range(0, cols, patch_width):
            if patch_idx < len(mask_indices) and mask_indices[patch_idx] == 0:
                # print(row, col, patch_idx)
                pre_new[row : row + patch_height, col : col + patch_width] = x[
...
def visualize_spec(pretrain="operaGT", data_source="ICBHI"):
    ...
    emb_enc, mask, ids_restore, _ = model.forward_encoder(
        x, mask_ratio=0.7, mask_2d=False
    )
    ...
    prediction_new = replace_generation(
        data[0], prediction, mask[0]
    )  # show the original patch for the non-masked pixel
