...
class CLAPWrapper:
    ...
    def __init__(
        self, model_fp: Path | str | None = None, version: str = "2023", use_cuda=False
    ):
        # Check if version is supported
        self.supported_versions = self.model_name.keys()
        if version not in self.supported_versions:
            raise ValueError(
                f"The version {version} is not supported. The supported versions are {str(self.supported_versions)}"
            )
        ...
        self.config_as_str = (
            Path(__file__).parent / f"configs/config_{version}.yml"
        ).read_text()
    ...
    def load_clap(self):
        ...
        if (
            "roberta" in args.text_model
            or "clip" in args.text_model
            or "gpt" in args.text_model
        ):
    ...
    def get_audio_embeddings_per_batch(self, audio_files, batch_size):
        ...
        return self._generic_batch_inference(
            self.get_audio_embeddings, audio_files, batch_size
        )
    ...
    def get_text_embeddings_per_batch(self, class_labels, batch_size):
        ...
        return self._generic_batch_inference(
            self.get_text_embeddings, class_labels, batch_size
        )
    ...
    def generate_caption(
        ...
        with torch.no_grad():
            ...
            prefix_embed = self.clapcap.clap_project(prefix).view(
                ...
                self.clapcap.gpt.transformer.wte.weight.shape[1],
            )
    ...
    def _generate_beam(
        ...
        with torch.no_grad():
            ...
            for i in range(entry_length):
                ...
                next_token_embed = self.clapcap.gpt.transformer.wte(
                    next_tokens.squeeze()
