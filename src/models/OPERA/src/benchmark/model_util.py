...
def extract_opera_feature(sound_dir_loc, pretrain="operaCE", input_sec=8, from_spec=False, dim=1280, pad0=False, sr=None, butterworth_filter=None, lowcut=200, highcut=1800):
    """
    ...
    with torch.no_grad():
        for audio_file in tqdm(sound_dir_loc):
            if MAE:
                data = get_split_signal_librosa("", audio_file[:-4], spectrogram=True, input_sec=input_sec, sample_rate=sr, butterworth_filter=butterworth_filter, lowcut=lowcut, highcut=highcut) if not from_spec else [audio_file[i:i+256] for i in range(0, len(audio_file), 256)]
                features = [
                    model.forward_feature(torch.tensor(np.expand_dims(x, axis=0), dtype=torch.float).to(device)).detach().cpu().numpy()
                    for x in data if x.shape[0] >= 16
                ]
                features_sta = np.mean(features, axis=0)
                opera_features.append(features_sta.tolist())
            else:
                data = get_entire_signal_librosa("", audio_file[:-4], spectrogram=True, input_sec=input_sec, pad=True, types='zero' if pad0 else None, sample_rate=sr, butterworth_filter=butterworth_filter, lowcut=lowcut, highcut=highcut)
                x = torch.tensor(np.expand_dims(data, axis=0), dtype=torch.float).to(device)
...
def initialize_pretrained_model(pretrain):
    if pretrain == "operaCT":
        ...
        model = mae_vit_small(
            ...
            img_size=(256,64), alpha=0.0, mode=0,
            use_custom_patch=False, split_pos=False,
            pos_trainable=False, use_nce=False,
            decoder_mode=1, mask_2d=False,
            mask_t_prob=0.7, mask_f_prob=0.3,
            no_shift=False
        ).to(device).float()
    else:
        raise NotImplementedError(f"Model not found: {pretrain}, please check the parameter.")
    return model
