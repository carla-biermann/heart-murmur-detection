...
def preprocess_task1(modality="cough"):
    ...
    for _, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        file = "/".join(
            ["datasets/covid19-sounds/0426_EN_used_task1", userID, folder, filename]
        )
...
def task1_downsample(downsampling_factor=5):
    ...
    for modality in ["cough", "breath"]:
        ...
        new_sound_dir_loc = np.concatenate(
            [
                downsampled_train_sound_dir_loc,
                sound_dir_loc[val_idx],
                sound_dir_loc[test_idx],
            ]
        )
        np.save(
...
def extract_and_save_embeddings_baselines(
    task=1, modality="cough", feature="opensmile"
):
