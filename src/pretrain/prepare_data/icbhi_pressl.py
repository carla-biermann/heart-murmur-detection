...
splits_data = pd.read_csv(
    "datasets/icbhi/ICBHI_challenge_train_test.txt",
    dtype=str,
    sep="\t",
    names=["fileID", "group"],
)
demographics_data = pd.read_csv(
    "datasets/icbhi/ICBHI_Challenge_demographic_information.txt",
    dtype=str,
    sep="\t",
...
def preprocess_cycle_spectrogram(input_sec=2):
    ...
    for i in tqdm(range(sound_dir_loc.shape[0])):
        ...
        sample_data = get_individual_cycles_librosa(
            "cycle",
            annotation_dict[fileID],
            "datasets/icbhi/ICBHI_final_database",
            fileID,
            SR,
            2,
        )
